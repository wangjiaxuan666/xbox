...
#'
#' The function from bioVenn R packages(https://cran.r-project.org/web/packages/BioVenn/index.html), I improve the data output and delete the `output` parameter.
#' on the next step , I want be add the ggplot2 on the function which draw venn plot. or change the postion of group title name whcih is on the circ point now.
#'
...
#' @import graphics grDevices plotrix
#' @examples
...
#' biovenn <- vennplot(list_x, list_y, list_z, subtitle="Example diagram", nrtype="abs")
#' @export
vennplot <- function(list_x = NULL,
                     list_y = NULL,
                     list_z = NULL,
                     title="Venn Plot",
                     subtitle="Creat diagram by R packages xbox",
                     t_f="serif", t_fb=2, t_s=1.5, t_c="black",
                     st_f="serif", st_fb=4, st_s=1.5, st_c="black",
                     xtitle="X Set", xt_f="serif", xt_fb=2, xt_s=1.5, xt_c="white",
                     ytitle="Y Set", yt_f="serif", yt_fb=2, yt_s=1.5, yt_c="white",
                     ztitle="Z Set", zt_f="serif", zt_fb=2, zt_s=1.5, zt_c="white",
                     nrtype="abs", nr_f="serif", nr_fb=2, nr_s=1, nr_c="black",
                     x_c="#02786A", y_c="#D23918", z_c="#4C8045", bg_c="white",
                     width=100, height=100, filename=NULL
){
  ...
  xyz <- length(list_xyz)
  list_all <- unique(c(x,y,z))
  xyz_all <- length(list_all)
  ...
  cat("Venn Data:","\n")
  print.data.frame(venn_data,row.names = FALSE)
  cat(paste(rep("-",getOption("width")),collapse=""),"\n")
  ...
  #Get file type:change by wangjaixuan0123
  if(!is.null(filename)){
    ...
    f = switch(ending,
               ...
               png = function(x, ...) png(x, units="px", ...),
               jpeg = function(x, ...) jpeg(x, units="px",  ...),
               jpg = function(x, ...) jpeg(x, units="px", ...),
               tiff = function(x, ...) tiff(x, units="px", ...),
               bmp = function(x, ...) bmp(x, units="px", ...),
